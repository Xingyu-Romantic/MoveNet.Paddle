...
class Task():
    def __init__(self, cfg, model):
        ...
        # scheduler
        self.scheduler = getSchedu(self.cfg['scheduler'], self.cfg['learning_rate'])
    ...
    def predict(self, data_loader, save_dir):
        ...
        with paddle.no_grad():
            ...
            for (img, img_name) in data_loader:
                ...
                ## debug
                heatmaps = output[0].numpy()[0]
                centers = output[1].numpy()[0]
                regs = output[2].numpy()[0]
                offsets = output[3].numpy()[0]
        ...
        with paddle.no_grad():
            for batch_idx, (imgs, labels, kps_mask, img_names) in enumerate(data_loader):
                ...
                # if sum(acc)/len(acc)==1: 
                    ...
                    hm = cv2.resize(np.sum(output[0][0].numpy(),axis=0),(192,192))*255
                    cv2.imwrite(os.path.join(save_dir,basename[:-4]+"_hm_pre.jpg"),hm)

                    hm = cv2.resize(np.sum(labels[0,:7,:,:].numpy(),axis=0),(192,192))*255
                    cv2.imwrite(os.path.join(save_dir,basename[:-4]+"_hm_gt.jpg"),hm)



                    img = np.transpose(imgs[0].numpy(),axes=[1,2,0])
                    img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                    h,w = img.shape[:2]
...
################ 
    def onTrainStep(self,train_loader, epoch):
        ...
        for batch_idx, (imgs, labels, kps_mask,img_names) in enumerate(train_loader):
            ...
            self.optimizer.clear_grad()#把梯度置零
    ...
    def onValidation(self, val_loader, epoch):
        ...
        with paddle.no_grad():
            for batch_idx, (imgs, labels, kps_mask, img_names) in enumerate(val_loader):
                ...
                ### evaluate
                # acc1 = myAcc(heatmap2locate(output[0].detach().numpy()), 
                #                 heatmap2locate(labels[:,:7,:,:].detach().numpy()))
                pre = movenetDecode(output, kps_mask,mode='output')
                gt = movenetDecode(labels, kps_mask,mode='label')
                acc = myAcc(pre, gt)
        ...
        save_name = 'e%d_valacc%.5f.pdparam' % (epoch+1,np.mean(right_count/total_count))
    ...
    #             print("\r" + str(i) + "/" + str(len(data_loader)), end="",flush=True)
    ...
    def modelLoad(self,model_path, data_parallel = False):
        ...
        if data_parallel:
            self.model = paddle.nn.DataParallel(self.model)


    def modelSave(self, save_name):
        paddle.save(self.model.state_dict(), os.path.join(self.cfg['save_dir'], save_name))
